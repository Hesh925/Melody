...
module.exports = {
	...
	execute: async (client, interaction) => {
		...
		if (type === "playing")   { client.user.setPresence({ activities: [ { name: text, type: ActivityType.Playing   } ] }); }

		if (type === "listening") { client.user.setPresence({ activities: [ { name: text, type: ActivityType.Listening } ] }); }

		if (type === "watching")  { client.user.setPresence({ activities: [ { name: text, type: ActivityType.Watching  } ] }); }
