...
module.exports = {
	...
	execute(Discord, client, colors, opusEncoder, voicePlayer, DJSVoice, nowPlaying, error) {
		...
		const errorAborted = new Discord.MessageEmbed()
			.setTitle("Error `Aborted`")
			.setColor(ezcolor.getColor("HEX", "red"))
			.setDescription(`Sorry, there was a problem with \`${ error.resource.metadata.title }\`. Skipping`);
		...
		client.guilds.cache.find(guild => guild.id === error.resource.metadata.guildId).channels.cache.find(channel => channel.id === error.resource.metadata.textCId).send("<@575844183546265609>", {embed: error403});
	}
};